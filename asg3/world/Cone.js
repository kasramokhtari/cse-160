...
class Cone {
  ...
  render() {
    ...
    // The cone vertex buffer holds only positions (no UVs), so we force the
    // fragment shader into solid-color mode and disable a_TexCoord so it
    // doesn't try to read tex coords from the cone buffer. Cube.render()
    // re-enables and re-sets a_TexCoord on its own.
